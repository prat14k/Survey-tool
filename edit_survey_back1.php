...
if(isset($_POST["submit"])) {
	...
	$q_res = mysql_query("SELECT question_id from questions where survey_q_id=".$oppp);
	while($q_row = mysql_fetch_array($q_res)){
		
			mysql_query("delete from options where options_q_id=".$q_row['question_id']);

	}
		mysql_query("delete from questions where survey_q_id=".$oppp);
		mysql_query("delete from survey_info where survey_id=".$oppp);
		mysql_query("drop table if exists ".$oppp."_responses");



	mysql_query("insert into survey_info values(null,'$org','$survey_title','$instruc',$attempts,$active,'$target_file','".$_SESSION['username']."','$d','$t')") or die(mysql_error());
